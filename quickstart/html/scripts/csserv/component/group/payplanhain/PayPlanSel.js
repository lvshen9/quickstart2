...
function selectPayPlan() {
    ...
    try {
	    var afterFunction = getElement("AFTER_ACTION").value;
	...
	if (payPlan == "G") {
	    hidden(getElement('intItemPart'),false);
	} else {
	    hidden(getElement('intItemPart'),true);
	}
...
function popupAfterAction(value) {
    ...
    for (var i=0;i<ds.length;i++) {
        ...
        if (!found) {
            ...
            getElement('X_CHOOSE').value = '<input type="checkbox" onClick="saveData();" name="item" id="item" checked="true"/>';
            getElement('PAYITEM_DETAIL_SET').value="";
