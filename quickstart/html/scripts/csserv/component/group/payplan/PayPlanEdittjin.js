...
function initCreatePayPlanEdit() {
	...
	{
		getElement('PLAN_ID').value = defaultPayPlans.get(i,'PLAN_ID','');
		...
		getElement('PAY_ITEMS').value = defaultPayPlans.get(i,'POP_PAY_ITEMS','');
		getElement('POP_PAY_ITEMS').value = defaultPayPlans.get(i,'POP_PAY_ITEMS','');
		getElement('POP_PAY_ITEMS_DESC').value = defaultPayPlans.get(i,'POP_PAY_ITEMS_DESC','');
		planTable.insertRow();
	}
...
function setAllPayPlans(obj)
{
	...
	{
		var plan = planTable.getRowData(planTable.table.rows[i], 'X_TAG,PLAN_ID,PLAN_TYPE,PLAN_NAME,PLAN_DESC,RSRV_STR2,POP_PAY_ITEMS,POP_PAY_ITEMS_DESC');
		if(plan)
		{
			if(plan.get('PLAN_TYPE') == 'C') {
...
function compareDataset(strNewValue,strOldValue){
    ...
    {   
    	...
    	for(var j=0;j<oldValueSet.length;j++){
    		...
    		if(newValueColumn.get('PLAN_ID','')==oldValueColumn.get('PLAN_ID','')){
	...
	for (var i=0;i<oldValueSet.length;i++) {
	    ...
	    for (var j=0;j<newValueSet.length;j++) {
	        ...
	        if(oldValueColumn.get('PLAN_ID','')==newValueColumn.get('PLAN_ID',''))
	        {
	        	isfound = "true";
	        	break;
	        }
